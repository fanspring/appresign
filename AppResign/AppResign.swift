...
class AppResign {
    ...
    var codesigningCerts: [CertificateFile] = []
    var connectedMobileDevices: [MobileDevice] = []
    var profileFilename: String?
    var curSigningCert: CertificateFile = CertificateFile()
    ...
    let iosDeployPath = "/usr/local/bin/ios-deploy"
    
    init() {
        populateConnectedMobileDevices()
        ...
        if securityResult.output.count >= 1 {
            ...
            for index in stride(from: 0, through: rawResult.count - 2, by: 1) {
                ...
                if(cerStrArray.count >= 2){
                    ...
                    let cer = CertificateFile(name: cerStrArray[1], ID: id)
                    output.append(cer)
                }
            }
        }
    ...
    func populateConnectedMobileDevices(){
        var output: [MobileDevice] = []
        let result = Process().execute(iosDeployPath, workingDirectory: nil, arguments: ["-c"])
        if result.output.count >= 1{
            var rawResult = result.output.components(separatedBy: "\n")
            if (rawResult.count <= 2){
                return
            }
            for index in stride(from: 1, to: rawResult.count-1, by: 1){
                let deviceUUid = rawResult[index].components(separatedBy: " ")[2]
                let deviceName = rawResult[index].components(separatedBy: "\'")[1]
                if (deviceName.count > 1 && deviceUUid.count == 40){
                    let device = MobileDevice(name: deviceName, UUID: deviceUUid)
                    output.append(device)
                }
            }
        }
        self.connectedMobileDevices = output
        Log("Found \(output.count) Connected Mobile Devices")
    ...
    func signingThread() {
        ...
        let signingCertificate = self.curSigningCert.ID
        var eggCount: Int = 0
