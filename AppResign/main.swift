...
func mainRoutine(_ input: String, output: String, certificate: String, provProfile: String, bundleId: String, displayName: String) {
    ...
    if support.contains(input.pathExtension) {
        if output.pathExtension == "ipa" {
            ...
            for i in 0..<n {
                print("[\(i)] \(appResign.codesigningCerts[i].name) \(appResign.codesigningCerts[i].ID)")
            }
            
            if n == 1 {
                appResign.curSigningCert = appResign.codesigningCerts[0]
                print("Use Certificate: \(appResign.curSigningCert.name) \(appResign.curSigningCert.name)\n")
            } else if (!certificate.isEmpty) {
                for c in appResign.codesigningCerts {
                    if (c.name == certificate || c.ID == certificate) {
                        appResign.curSigningCert = c
                        break
                    }
                }
                if (appResign.curSigningCert.ID.isEmpty) {
                    print("Can't find certificate: \(certificate)\n")
                    exit(2)
                }
                print("Use Certificate: \(appResign.curSigningCert.name) \(appResign.curSigningCert.ID)\n")
            } else {
                while true {
                    let r = Int(raw_input())
                    if (r != nil) && (r! >= 0) && (r! < n) {
            ...
            for i in 0..<n {
                ...
                var supportedDeviceNameStr = ""
                for device in appResign.connectedMobileDevices{
                    if profile.isMobileDeviceSupported(device){
                        supportedDeviceNameStr = "\(supportedDeviceNameStr),\(device.name)"
                    }
                }
                print("[\(i)] \(profile.name) (\(profile.teamID))----[supported connected device\(supportedDeviceNameStr)]")
            }
