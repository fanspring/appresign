...
struct ProvisioningProfile {
        ...
        if taskOutput.status == 0 {
            ...
            if let results = try? PropertyListSerialization.propertyList(from: self.rawXML.data(using: String.Encoding.utf8)!, options: PropertyListSerialization.MutabilityOptions(), format: nil) as AnyObject {
                ...
                if let expirationDate = results.value(forKey: "ExpirationDate") as? Date,
                    ...
                    let periodIndex = applicationIdentifier.index(of: "."),
                    let provisionedDevices = results.value(forKey: "ProvisionedDevices") as? Array<String>,
                    let isXcodeManaged = results.value(forKey: "IsXcodeManaged") as? Bool{
    ...
    func isMobileDeviceSupported(_ device:MobileDevice) -> Bool {
        for uuid in provisionedDevices {
            if(uuid == device.UUID){
                return true
            }
        }
